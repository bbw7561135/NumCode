...
//左移运算符<< 将一个数的各二进制位全部左移若干个位置 
//友移运算符>> 将一个数的各二进制位全部右移若干个位置
//var<<左移位数 左移过程中 高位左移后溢出舍弃 左移一位相当于var乘以2 移二位相当于var乘以2的平方
//右移移到右端低位也被舍弃 对于无符号数高位补0 右移动1位等于var除以2 右移2位等于var除以4
//右移需要注意符号位问题


//break只能用于循环和switch语句
//数组作为函数参数 传递的是地址 也就是首元素地址 所以应该再多一个参数表示大小
//
...
//编译时分配存储空间的变量称为静态存储变量 static 程序运行整个期间都存在 不论其作为全局还是局部变量
//静态变量定义和初始化都在编译时进行

//函数本质上是全局的 但根据能否被其他源文件调用 分为内部函数和外部函数

//C语言精彩编程百例可以作为handbook使用 基本功能都讲到了 比较实用

//最关键的是遇到问题要想到如何去找方法解决 因为问题千万种 你不可能记住所有问题的答案 但你要学会如何去找寻问题的解决方案
